...
Also merges all matched files into a single combined PDF per group.

Build into .exe (Windows):
    pip install pyinstaller pytesseract pdfplumber Pillow pypdf
...
def get_document_info(path: Path, stop_event: threading.Event) -> tuple[set[str], set[str]]:
    """Return (po_numbers, dn_numbers) from the PDF, using OCR fallback."""
...
def merge_pdfs(sources: list[Path], dest: Path, log_fn) -> bool:
    """
    Merge *sources* (in order) into a single PDF at *dest*.
    Returns True on success.
    """
    if not PYPDF_AVAILABLE:
        log_fn("    ⚠ pypdf not installed — skipping merge (pip install pypdf)")
        return False
    if not sources:
        return False
    try:
        ...
        for src in sources:
            try:
                ...
                log_fn(f"    ⚠ Could not read {src.name} for merge: {e}")
        if len(writer.pages) == 0:
            log_fn("    ⚠ No pages to merge.")
            return False
...
def run_matching(
    ...
    try:
        ...
        msg_queue.put(("progress", (total, total, "Organising and merging files…")))
        ...
        log(f"\n── Organising and merging matched files ──")
        ...
        for po in sorted(common):
            ...
            # Collect all D/N numbers across all files for this PO
            all_dns: set[str] = set()
            ...
            if use_dn_folders:
                log(f"\n  [PO {po}]  {len(unique_entries)} files, "
                    f"D/N: {sorted(all_dns)} → D/N subfolders")

                # Group files by D/N
                dn_groups: dict[str, list[Path]] = {}   # dn → [paths in order]
                unmatched: list[Path] = []

                for src, dns in unique_entries:
                    if dns:
                        for dn in sorted(dns):
                            dn_groups.setdefault(dn, []).append(src)
                    else:
                        unmatched.append(src)

                # Process each D/N group
                for dn in sorted(dn_groups):
                    if stopped():
                        msg_queue.put(("stopped", "Stopped by user."))
                        return
                    ...
                    group_files = dn_groups[dn]

                    # Copy individual files
                    for src in group_files:
                        dst = _unique_path(dn_dest / src.name)
                        shutil.copy2(src, dst)
                        log(f"    [{po}/DN{dn}] {src.name}")
                        copied += 1

                    # Merge into combined PDF
                    merged_name = f"{po}_DN{dn}.pdf"
                    merged_dest = dn_dest / "combined" / merged_name
                    if merge_pdfs(group_files, merged_dest, log):
                        log(f"    [{po}/DN{dn}] ✦ merged → {merged_name}")
                        merged += 1

                # Files with no D/N go to DN_unknown
                if unmatched:
                    unk_dest = po_dest / "DN_unknown"
                    unk_dest.mkdir(parents=True, exist_ok=True)
                    for src in unmatched:
                        dst = _unique_path(unk_dest / src.name)
                        shutil.copy2(src, dst)
                        log(f"    [{po}/DN_unknown] {src.name}  ⚠ no D/N detected")
                        copied += 1
                    merged_name = f"{po}_DN_unknown.pdf"
                    merged_dest = unk_dest / "combined" / merged_name
                    if merge_pdfs(unmatched, merged_dest, log):
                        log(f"    [{po}/DN_unknown] ✦ merged → {merged_name}")
                        merged += 1

            else:
                # Flat — no D/N subfolders
                log(f"\n  [PO {po}]  {len(unique_entries)} file(s) → flat folder")
                group_files = [src for src, _ in unique_entries]

                for src in group_files:
                    dst = _unique_path(po_dest / src.name)
                    shutil.copy2(src, dst)
                    log(f"    [{po}] {src.name}")
                    copied += 1

                # Merge — named just with PO number (no D/N)
                merged_name = f"{po}.pdf"
                merged_dest = po_dest / "combined" / merged_name
                if merge_pdfs(group_files, merged_dest, log):
                    log(f"    [{po}] ✦ merged → {merged_name}")
                    merged += 1

        summary = (
            f"✓ Matched {len(common)} PO number(s).\n"
            f"  Copied {copied} file(s),  created {merged} merged PDF(s).\n"
...
class POMatcherApp(tk.Tk):
    ...
    SUCCESS    = "#3ecf8e"
    WARNING    = "#f6a623"
    MERGE_COL  = "#b57bee"   # purple for merge lines
    TEXT       = "#e8ecf4"
    MUTED      = "#7a859e"
    LOG_BG     = "#141820"
    LOG_FG     = "#c5cede"
    ...
    def _build_ui(self):
        ...
        tk.Label(hdr,
                 text="Match & merge invoice PDFs to purchase orders · groups by D/N when needed",
        ...
        self._log.tag_config("merge",   foreground=self.MERGE_COL)
        ...
        try:
            while True:
                ...
                if msg_type == "log":
                    ...
                    elif payload.startswith("✓") or (
                        "[" in payload and "⚠" not in payload and "✦" not in payload
                    ):
                        self._log_write(payload + "\n", "success")
                    elif "⚠" in payload or "unknown" in payload:
                        self._log_write(payload + "\n", "warn")
                    ...
                    self._status.configure(text=f"  Done.")
...
def _check_dependencies() -> list[str]:
    ...
    if not PYPDF_AVAILABLE:
        issues.append("pypdf not installed (needed for merging)  →  pip install pypdf")
    try:
